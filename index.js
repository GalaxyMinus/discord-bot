    ...
    */
    ...
    if (message.content.startsWith('!help')) {
            ...
            if (message.content === '!help' || message.content ===  '!help 1' || message.content === '!help utility') {
                ...
                const embed = new MessageEmbed()
                    ...
                    .addFields(
                        ...
                        { name: '!kill [mention]', value: 'Kill the user that you mention D:' },
        ...
        try {
            ...
            if (message.content === '!shutdown' && message.author.id === '853310508341526548' || message.content === '!restart' && message.author.id === '853310508341526548' || message.content === '!shutdown' && message.author.id === '853310508341526548') {
        ...
        if (message.content.startsWith ('!hug')) {
            ...
            if (message.content.startsWith ('!kill')) {
                try {
                    ...
                    return message.channel.send(`You need to mention someone to kill <@${message.author.id}>!`);
                    }
                    else if (undefined === message.mentions.users.first()) {
                    return message.channel.send(`You need to mention someone to kill <@${message.author.id}>!`);
                    }
                    else if (message.mentions.users.first().id === message.author.id) {
                        console.log("kill command with self mention ran successfully");
                        return message.channel.send('No! Dont try it ' + '<@' + message.author.id + '>! :(');
                    }
                    else if (message.mentions.users.first().id != 'undefined') {
                            const user = client.users.cache.get("853310508341526548");
                            const Embed = new MessageEmbed()
                        // create a array of main colors for the embed
                        .setColor(['#ff0000', '#00ff00', '#0000ff', '#ffff00', '#00ffff', '#ff00ff', '#000000', '#ffffff'][Math.floor(Math.random() * 8)])
                        .setDescription('<@' + message.mentions.users.first().id  + '>' + ' has been killed by ' + '<@' + message.author.id + '>! ' + 'D:')
                        .setImage([
                        'https://c.tenor.com/NbBCakbfZnkAAAAC/die-kill.gif','https://c.tenor.com/_aMkVJcxClIAAAAC/yu-yu-hakusho-anime.gif',
                        'https://c.tenor.com/FJsjk_9b_XgAAAAC/anime-hit.gif','https://c.tenor.com/vZnHUMCUU6AAAAAC/handa-naru-barakamon.gif',
                        'https://c.tenor.com/p_mMicg1pgUAAAAC/anya-forger-damian-spy-x-family.gif','https://c.tenor.com/fz-V6dZ1PiQAAAAC/how-to-raise-a-boring-girlfriend-saenai.gif',
                        'https://c.tenor.com/BoYBoopIkBcAAAAC/anime-smash.gif','https://c.tenor.com/vv1mgp7IQn8AAAAC/tgggg-anime.gif'
                        ][Math.floor(Math.random() * 8)])
                        .setTimestamp()
                        .setFooter({ text: 'Created by: '+user.username+'#'+user.discriminator, iconURL: user.displayAvatarURL({ format: 'png'}) });
                        console.log("kill command ran successfully");
                        return message.channel.send({ embeds: [Embed] });
                        }
                    else {
                        console.error('error in kill command???');
                    }
                } catch (error) {
                    console.log(error);
                }
            ...
            if (message.content.startsWith ('!kill')) {
                try {
                    if (message.content === '!kill') {
                    return message.channel.send('You need to tag someone ' + '<@' + message.author.id + '>...');
                    }
                    else if (undefined === message.mentions.users.first()) {
                        return message.channel.send('You need to tag someone ' + '<@' + message.author.id + '>...');
                    }
            ...
            if (message.content.startsWith ('!sex')) {
                try {
                    if (message.content === '!sex') {
                        ...
                        console.log("Sex command with self mention ran successfully");
